...
class WeeklyFee(BaseModel):
    ...
    price_per_week: float = Field(..., description="Price of the course")
...
class AdditionalFee(BaseModel):
    ...
    price_per_week: float = Field(..., description="Price of the fee")
    currency: str = Field(..., description="Currency of the price in ISO 4217 format (e.g., 'EUR', 'USD')")
...
class AccommodationFee(BaseModel):
    name: str = Field(..., description="Name or description of the accommodation")
...
class Course(BaseModel):
    ...
    weekly_course_fees: List[WeeklyFee] = Field(..., description="List of prices for the course")
    requirements: Optional[str] = Field(None, description="Course requirements")
    additional_fees: Optional[List[AdditionalFee]] = Field(default_factory=list, description="Additional fees")
...
class Location(BaseModel):
    ...
    accommodation_fees: List[AccommodationFee] = Field(..., description="List of accommodations")
    suplement_fees: Optional[List[SupplementFee]] = Field(default_factory=list, description="List of supplements")
    additional_fees: Optional[List[AdditionalFee]] = Field(default_factory=list, description="Additional fees")
