...
def merge_location_details(existing: dict, new: dict):
    ...
    existing["courses"] = merge_courses(
        existing.get("courses", []), new.get("courses", [])
    )
    existing["accommodations"] = merge_accommodations(
        existing.get("accommodations", []), new.get("accommodations", [])
    )
...
def merge_schools(school1: dict, school2: dict) -> dict:
    ...
    merged["locations"] = merge_locations(
        merged.get("locations", []), school2.get("locations", [])
    )
    return merged
